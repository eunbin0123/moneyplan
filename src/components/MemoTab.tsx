  ...
  return (
    <div className="bg-white border-2 border-black p-5 rounded-none geo-shadow space-y-5">
      ...
      <div className="flex items-center justify-between pb-3 border-b-2 border-black">
        <div>
          <h3 className="text-sm font-black text-black">{shortMonthLabel} 통합 메모</h3>
        </div>

        {/* Saved check status indicator */}
        <span
          className={`inline-flex items-center gap-1 text-[10px] font-mono font-bold uppercase text-white bg-black border border-black px-2.5 py-1 rounded-none transition-opacity duration-300 ${
            savingIndicator ? "opacity-100" : "opacity-0 pointer-events-none"
          }`}
        >
          <Check className="h-3 w-3 text-emerald-400" /> 저장됨
        </span>
      </div>
